...
4. Call `image_generate` once per needed visual. Inspect the resulting PNG with the available image viewer before placing it. Correct specific visible defects if another generation is needed. Reuse an existing suitable image instead of generating duplicates.
