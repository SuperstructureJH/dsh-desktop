...
describe('PPT catalog migration', () => {
  it.each(['DSH-PPT-AUTHORING-20260906-V2', 'DSH-PPT-AUTHORING-20260907-V3'])('refreshes %s with the image-template workflow', async marker => {
    const f = await fixture()
    const agent = await f.agent()
    await f.toggle(agent, true)
    ...
    expect(JSON.stringify(messages)).toContain('dsh.template-images/v1')
  ...
  it('refreshes an old automatic Skill snapshot when PPT remains active', async () => {
    ...
    const messages = automaticMessages(agent).filter(m => m.source.plugin === 'dsh-ppt-skill')
    expect(messages).toHaveLength(1)
    expect(JSON.stringify(messages)).toContain('DSH-PPT-AUTHORING-20260914-IMAGE-TEMPLATES-V1')
