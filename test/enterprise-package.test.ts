...
describe('DSH Desktop enterprise package', () => {
  it('registers the enterprise package and login protocol in the desktop build', async () => {
    ...
    const patchManifest = await readFile(
    ...
    expect(packageJson.dependencies?.['dsh-desktop-enterprise']).toBe(
      'file:packages/dsh-desktop-enterprise'
    )
    expect(packageJson.build?.protocols?.flatMap((entry) => entry.schemes ?? [])).toContain(
      'dsh-desktop'
    )
    expect(patchManifest).toContain('id: dsh-desktop-enterprise')
    expect(dshPatch).toContain('"dsh-desktop-enterprise": "0.2.0"')
  })
