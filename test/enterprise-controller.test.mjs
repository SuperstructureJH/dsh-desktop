...
describe('enterprise controller', () => {
  ...
  it('owns the complete browser login, provider registration, refresh state, and logout lifecycle', async () => {
    ...
    expect(callbackHtml).toContain('授权已完成，页面将自动返回毕昇首页。')
    expect(callbackHtml).toContain(`http-equiv="refresh" content="10;url=${origin}"`)
    ...
    expect(enterpriseAdapter.providerInfo('bisheng-enterprise')).toEqual({
      id: 'bisheng-enterprise',
      name: '毕昇演示企业'
    })
