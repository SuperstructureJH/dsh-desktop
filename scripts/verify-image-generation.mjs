...
const mock = createServer(async (request, response) => {
  ...
  response.end(JSON.stringify(valid ? probe ? { error: { code: 'MissingParameter', message: 'The request is missing a required parameter: prompt.' } } : { id: DEFAULTS.openai.model } : { error: { message: 'Invalid API key' } }))
})
...
try {
  ...
  console.log('PASS: Host composition, authentication, origin checks, redacted settings, save success/failure, exactly one non-generating request per save.')
