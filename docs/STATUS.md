...
## PR #436 Green Pulse 模板：V0.9.1 冲突处理 — 2026-09-17

- 基线：远端 `V0.9.1@97e02f8bf773a3b28bd4c6c05f874156be8ab5ab`；直接继承 PR #423 的 `7792e7f60819f8787b22b53af96609a8aa98685c`。工作树 `.worktrees/dsh-desktop-green-pulse-v091`，分支 `codex/green-pulse-template-v091`，按锁文件重新安装后的 Harness 为 `0.1.5-rc.2`。
- PR 分工：#423 交付个人模板上传和复用；#436 的独立提交交付 Green Pulse 22 页模板、17 个图片槽与 11 个遮罩、模板构建流程和配套图表导出修复。评审与合并顺序为 #423 → #436。
- 冲突处理：沿用 #423 已验证的个人模板和 HTTP 路由实现，在其上重放模板与导出提交；从组合源码重建归档并同步锁文件与哈希清单。个人模板管理组件、基础库和上传回归与 #423 共用同一提交历史。
- 自动验证 `PASS`：115 个测试文件、965 项测试，覆盖旧会话向图片模板指令迁移、17 套模板目录和既有选择保留；TypeScript、生产构建、16 套既有模板源检查与 `git diff --check` 通过。
- 分发包专项验证 `PASS`：全新目录、预览、选择持久化、工具输出、22 页工作区副本、17 个图片槽、11 个遮罩、路径边界、源文件隔离、变更审计和共享生图工作流。实际使用本分支重建并安装的 `node_modules/dsh-ppt`。
- CI：继承 #423 的 `V0.9.1` PR 工作流。原生安装包、真实生图服务商会话、PowerPoint/WPS 编辑保存重开和部署为 `NOT_RUN`。

## 历史 Green Pulse 内置可编辑配图模板 — 2026-09-15
