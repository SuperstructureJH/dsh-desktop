...
- Slides 源码分支：`codex/desktop-plugin-compatibility`，发布重构基点 `b77dc58`
...
└── conversation.composer.dock：Slides 入口与模板目录
        │
        ▼
...
模板目录位于输入框下方。选中模板后，缩略图占据输入区左侧的小区域，正文从右侧同行开始；空态占位词贴近输入区顶部。任务被会话接受后，缩略图、Slides 入口和模板目录一起收起。图库使用独立纵向滚动和 overscroll 边界。
...
```text
4aec563ee15a632333b2d7f73faba98d3825246500e7eaa11a41f841a82fc873  33181364  dsh-workbuddy-ppt-0.1.1-rc.2-desktop-20260902-slides-only.tgz
cb9ca9211d3b33dd70d6b3fa23dc0bfa593edcbade81de1fdf274ad044b8fca7   8814031  deepseek-ai-dsh-experimental-office-ppt-standard-adapter-0.1.1-rc.2-desktop-20260902-slides-only.tgz
```

本地 macOS arm64 体验包：

```text
c76b042d9566675666b58ef00972069c8266604c1a41544953c7c08eaa05a5d6  470660124  ppt-capability-slides-only-mac-arm64.zip
```

体验包采用 ad-hoc 签名，仅用于本轮验证和内部体验。相较上一版 485,295,398 字节的候选 ZIP，本版减少 14,635,274 字节；332 张模板源页与约 564 MiB 的腾讯 Slides 平台运行时仍是包体主体。
...
| Slides 源码 bundle | PASS | Host、Client 与标准适配器完成干净 bundle；发布归档只含 Slides 路线 |
...
| macOS arm64 目录包 | PASS | 打包输入通过严格运行时校验；Electron Builder 完成 arm64 目录包与 ZIP；App ad-hoc 签名严格校验通过，ZIP 完整性通过；代码插件约 46 MiB，Slides 运行时约 564 MiB |
| 原生 Slides 回归 | NOT_RUN | 新包生成后复核空态、模板选中态、任务开始收起与真实生成 |
